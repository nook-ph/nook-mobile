...
class _CrawlDetailPageState extends State<CrawlDetailPage> {
  ...
  Widget build(BuildContext context) {
    return BlocProvider<ShareCardCubit>.value(
      ...
      child: BlocProvider<CrawlDetailCubit>.value(
        value: _cubit,
        child: BlocConsumer<CrawlDetailCubit, CrawlDetailState>(
          listener: (context, state) {
            ...
            if (detail != null) {
              context.read<ShareCardCubit>().setShareCardKey(_shareCardKey);
              context.read<ShareCardCubit>().loadData(detail.crawl.id);
            }
          },
                              ...
                              if (detail?.userProgress != null) ...[
                                ...
                                Padding(
                                  ...
                                  child: CrawlProgressCard(
                                    claimedStops:
                                        detail!.userProgress!.totalStamps,
                                    totalStops: detail.stops.length,
                                    currentTierName: detail
                                            .userProgress!.highestTier
                                            ?.name ??
                                        '',
                                  ),
                                ),
                              ],
                              if (detail?.tiers != null &&
                                  detail!.tiers.isNotEmpty) ...[
                                const SizedBox(height: 16),
                                Padding(
                                  padding: const EdgeInsets.symmetric(
                                    horizontal: 22,
                                  ),
                                  child: CrawlTiersCard(
                                    tiers: detail.tiers,
                                  ),
                                ),
                              ],
                              if (detail != null &&
                                  detail.stops.isNotEmpty) ...[
                                const SizedBox(height: 16),
                                Padding(
                                  padding: const EdgeInsets.symmetric(
                                    horizontal: 22,
